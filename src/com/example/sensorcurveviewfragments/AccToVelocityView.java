...
public class AccToVelocityView extends MyCurveView {
	{
		_type = new int[] { Sensor.TYPE_LINEAR_ACCELERATION };
	}
	// float _dt=0;
...
//		System.out.println("dt: "+dt+", "+event.timestamp+", "+_lastTimeStamp);
		...
		for (int i = 0; i < 3; i++) {
			...
			if (_accCurveEnabled) {
				...
				_canvas.drawLine(_lastX, base + _lastAccVals[i] * factor, newX,
						base + _accVals[i] * factor, _paint);
			}
			if (_velocityCurveEnabled) {
				_paint.setPathEffect(oldPaint.getPathEffect());
				_canvas.drawLine(_lastX, base + _lastVelocityVals[i] * factor,
						newX, base + _velocityVals[i] * factor, _paint);
			}
